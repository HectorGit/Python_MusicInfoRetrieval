function output = compressor_james(input, sr, ratio, threshold, attack_tau, release_tau)
...
%   param release_tau: Release time in ms
    ...
    for n = 1:input_length
        ...
        if abs(input(n)) < noise_floor
            ...
            input_gain(n) = 20*log10(abs(input(n)));
        end
        ...
        if input_gain(n) >= threshold
            ...
            output_gain(n) = threshold + ((input_gain - threshold) / ratio);
        else
